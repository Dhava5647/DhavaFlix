    ...
    return (
        <>
            <Head>
                ...
                <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context":"https://schema.org","@type":"WebSite","name":"DhavaFlix","url":"${SITE_URL}","potentialAction":{"@type":"SearchAction","target":"${SITE_URL}/search?q={search_term_string}","query-input":"required name=search_term_string"}}` }} />
            </Head>
...
function HeroSection({ item, onPlayNow }) {
    ...
    return (<motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative min-h-[50vh] md:min-h-[calc(85vh-5rem)] flex items-center"><div className="absolute inset-0"><Image src={`${IMAGE_BASE_URL}w1280${item.backdrop_path}`} layout="fill" objectFit="cover" className="object-top" alt={fullItem.title} priority /><div className="absolute inset-0 hero-gradient"></div></div><div className="relative z-10 w-full max-w-7xl px-4 sm:px-6 lg:px-8"><div className="max-w-xl"><h1 className="text-3xl md:text-6xl font-bold hero-text-shadow">{fullItem.title}</h1><p className="mt-4 text-sm md:text-lg max-w-lg hero-text-shadow line-clamp-2 md:line-clamp-3">{item.overview}</p><div className="mt-6 flex space-x-4"><button onClick={() => onPlayNow(fullItem)} className="bg-white text-black font-semibold py-2 px-5 rounded flex items-center hover:bg-gray-200 transition duration-300 hover:scale-105">▶ Play</button><Link href={detailUrl}><a className="bg-gray-700/80 font-semibold py-2 px-5 rounded flex items-center hover:bg-gray-600/70 transition duration-300 hover:scale-105">More Info</a></Link></div></div></div></motion.section>);
}
